...
public class ProcessUtil {
    ...
    public static String getForegroundProcessName() {
        ...
        if (android.os.Build.VERSION.SDK_INT > android.os.Build.VERSION_CODES.LOLLIPOP) {
            ...
            if (list.size() > 0) {// 有"有权查看使用权限的应用"选项
                try {
                    ApplicationInfo info = packageManager.getApplicationInfo(SiberiaDanteLib.getContext().getPackageName(), 0);
                    AppOpsManager aom = (AppOpsManager) SiberiaDanteLib.getContext().getSystemService(Context.APP_OPS_SERVICE);
                    if (aom.checkOpNoThrow(AppOpsManager.OPSTR_GET_USAGE_STATS, info.uid, info.packageName) != AppOpsManager.MODE_ALLOWED) {
                        SiberiaDanteLib.getContext().startActivity(intent);
                    }
                    if (aom.checkOpNoThrow(AppOpsManager.OPSTR_GET_USAGE_STATS, info.uid, info.packageName) != AppOpsManager.MODE_ALLOWED) {
                        ToastUtil.showTextLong("getForegroundApp---没有打开\"有权查看使用权限的应用\"选项");
                        return null;
                    }
                ...
                ToastUtil.showTextLong("getForegroundApp----无\"有权查看使用权限的应用\"选项");
            }
        }
        return null;
    }
    ...
    public static Set<String> getAllBackgroundProcesses() {
        ActivityManager am = (ActivityManager) SiberiaDanteLib.getContext().getSystemService(Context.ACTIVITY_SERVICE);
