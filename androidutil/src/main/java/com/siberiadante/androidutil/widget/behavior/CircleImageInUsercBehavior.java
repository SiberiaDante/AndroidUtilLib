...
public class CircleImageInUsercBehavior extends CoordinatorLayout.Behavior<SDCircleImageView> {
    ...
    @Override
    public boolean layoutDependsOn(CoordinatorLayout parent, SDCircleImageView child, View dependency) {
        return dependency instanceof DisInterceptNestedScrollView;
    }

    //当dependency变化的时候调用
    @Override
    public boolean onDependentViewChanged(CoordinatorLayout parent, SDCircleImageView child, View dependency) {
        //初始化一些基础参数
     ...
     * @param dependency
     */
    private void init(CoordinatorLayout parent, SDCircleImageView child, View dependency) {
        if (mStartAvatarY == 0) {
